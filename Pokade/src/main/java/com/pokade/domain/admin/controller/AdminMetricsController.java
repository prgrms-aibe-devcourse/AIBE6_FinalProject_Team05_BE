...
public class AdminMetricsController {
    ...
    // 어드민 대시보드 지표 카드 + 시계열 차트 데이터를 한 번에 반환한다.
    // Prometheus/Grafana는 이 응답의 데이터 원본일 뿐 화면에는 노출되지 않는다 - FE가 자체 컴포넌트로 그린다.
    @GetMapping("/dashboard")
    public ApiResponse<AdminDashboardResponse> getDashboard() {
        return ApiResponse.ok(adminMetricsService.getDashboard());
    }
}
